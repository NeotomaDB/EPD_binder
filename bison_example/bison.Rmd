...
```{r setup, echo=FALSE}
options(warn = -1)
pacman::p_load(neotoma2, dplyr, ggplot2, sf, geojsonsf, leaflet, DT, maptools, maps)
```
...
allSampDF <- allSampDF %>% 
  mutate(era = ifelse(ageyounger >= 11700, "Pleistocene", "Holocene")) %>% # Assign a new column with the values Pleistocene or Holocene based on ageyounger
...
10. Let's build a Convex Hull for the Bison data based on Time Period:

```{r convexHull}
bison_hull <- allSampDF %>%
  group_by(era) %>%
  slice(chull(long, lat)) # chull computes the convex hull using the xy points given to it
bison_hull
```
11. Let's plot the result:

```{r}
world <- map_data("world")
ggplot(allSampDF, aes(x = long, y = lat, colour=era)) + 
              geom_map(data = world, 
                       map = world,
                       aes(map_id = region),
                       color = "black",
                       ...
                       inherit.aes = FALSE) +
              geom_point() + 
              geom_polygon(data=bison_hull, alpha=.2) +
              theme_bw() +
              ggtitle("Bison in each Era") +
              xlab("Longitude") + 
              ylab("Latitude") + 
              coord_sf(xlim=c(-175, -80),ylim=c(20,72))
```
10.b We can try with a facet plot as well:
...
ggplot(allSampDF, aes(x = long, y = lat)) + 
              ...
              ylab("Latitude") 
```
10.b You can also use baseR to create similar plots! 
