...
```{r getMaule, message = FALSE}
maule <- get_downloads(19818)
maule_chron <- chronologies(maule)

maule_chron %>% as.data.frame() %>% 
  DT::datatable(data = ., 
...
controls <- chroncontrols(maule) %>% 
  dplyr::filter(chronologyid == 11463) %>% 
  arrange(depth)
...
predictDepths <- samples(maule) %>%
  dplyr::select(depth, analysisunitid) %>% 
  unique() %>% 
  arrange(depth)
...
newChronMaule <- set_chronology(agemodel = "Bchron model",
                                contact = creators,
                                isdefault = 1,
...
newChronMaule$notes <- 'newChron <- Bchron::Bchronology(ages = controls$chroncontrolage,
                                ageSds = abs(controls$agelimityounger - 
                                               controls$chroncontrolage),
                                calCurves = c("normal", rep("intcal20", 4)),
                                positionThicknesses = controls$thickness,
...
1. we have a collectionunit in `maule` that is accessible at `maule[[1]]$collunits`.
...
maule[[1]]$collunits[[1]] <- add_chronology(maule[[1]]$collunits[[1]], 
                                            newChronMaule, 
                                            newSampleAges)
```
...
newages <- samples(maule) %>%
  dplyr::select(depth, analysisunitid, age) %>% 
  unique() %>% 
  arrange(depth) %>% 
  mutate(agecat = "new")

maule[[1]]$collunits[[1]]$chronologies <- set_default(maule[[1]]$collunits[[1]]$chronologies,
                                                      11463)
plotforages <- samples(maule) %>%
  dplyr::select(depth, analysisunitid, age) %>% 
  unique() %>% 
  arrange(depth) %>% 
  mutate(agecat = "old") %>% 
...
1. Download a single record (the Maule record using `get_downloads()`)
